...
```{r, warning=FALSE,message=FALSE, echo= FALSE}
#Functions for use
simulate_forest <- function(dbh, p_evergreen = 0.5) {
  # Function to assign tree types based on probability of evergreen
  assign_tree_type <- function(dbh, p_evergreen) {
    sample(c("evergreen", "deciduous"), size = length(dbh), replace = TRUE, prob = c(p_evergreen, 1 - p_evergreen))
  }
  
  # Assign types to each tree
  tree_types <- assign_tree_type(dbh, p_evergreen)
  ...
  calculate_height <- function(dbh, type) {
    if (type == "evergreen") {
      (a_evergreen * dbh)^b_evergreen
    } else {
      (a_deciduous * dbh)^b_deciduous
    }
  }
  ...
  # Function to calculate crown diameter based on height and DBH
  calculate_crown_diameter <- function(H, dbh) {
    CA <- ((dbh / (a_biome * exp(g_biome / 2)))^(1 / b_biome)) / H
    d_CA <- 2 * sqrt(CA / pi)  # Convert from crown area to diameter
    d_CA
  }
  
  # Calculate crown diameter for each tree
  crown_diameters <- mapply(calculate_crown_diameter, heights, dbh)
